...
spatial_locs_slice1 = pd.read_csv("/home/zhaoshangrui/xuxinyu/datasets/mouse_hippocampus_data/mouse_hippocampus_data_Slide-seqV2/Puck_200115_08_bead_locations.csv", index_col=0)
expression_slice1 = pd.read_csv("/home/zhaoshangrui/xuxinyu/datasets/mouse_hippocampus_data/mouse_hippocampus_data_Slide-seqV2/Puck_200115_08.digital_expression.txt", sep='\t', index_col=0)

data_slice1 = anndata.AnnData(
    X=expression_slice1.T
)
data_slice1.obsm["spatial"] = spatial_locs_slice1.values
data_slice1 = process_data(data_slice1, n_top_genes=3000)


spatial_locs_slice2 = pd.read_csv("/home/zhaoshangrui/xuxinyu/datasets/mouse_hippocampus_data/mouse_hippocampus_data_Slide-seqV2/Puck_191204_01_bead_locations.csv", index_col=0)
expression_slice2 = pd.read_csv("/home/zhaoshangrui/xuxinyu/datasets/mouse_hippocampus_data/mouse_hippocampus_data_Slide-seqV2/Puck_191204_01.digital_expression.txt", sep='\t', index_col=0)

data_slice2 = anndata.AnnData(
    X=expression_slice2.T
)
...
nonzerovar_idx = np.intersect1d(
    np.where(np.array(data_slice1.X).var(0) > 0.1)[0],
    np.where(np.array(data_slice2.X).var(0) > 0.1)[0],
)
...
print(data_slice1.shape, data_slice2.shape)
# (4000, 24) (4000, 24)
data_slice1.write("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/data_slice1.h5ad")
data_slice2.write("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/data_slice2.h5ad")
...
for t in range(N_EPOCHS):
    ...
    if t % PRINT_EVERY == 0 or t == N_EPOCHS - 1:
        ...
        ax2.invert_yaxis()
        plt.draw()
        plt.savefig("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/slideseq_alignment.png")
        plt.pause(1 / 60.0)
        pd.DataFrame(curr_aligned_coords).to_csv("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/aligned_coords_slideseq.csv")
        pd.DataFrame(view_idx["expression"]).to_csv("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/view_idx_slideseq.csv")
        pd.DataFrame(X).to_csv("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/X_slideseq.csv")
        pd.DataFrame(Y).to_csv("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/Y_slideseq.csv")
        data.write("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/data_slideseq.h5")
        if model.n_latent_gps["expression"] is not None:
            curr_W = model.W_dict["expression"].detach().numpy()
            pd.DataFrame(curr_W).to_csv("/home/zhaoshangrui/xuxinyu/GPSA/mouse_hippocampus/result/W_slideseq.csv")
plt.close()
end_time = time.time()
