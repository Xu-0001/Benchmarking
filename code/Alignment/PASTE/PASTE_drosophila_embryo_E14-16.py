...
for i in range(len(slices_ID)-1):
    ...
    pis.append(pi)
    np.savetxt('/home/zhaoshangrui/xuxinyu/PASTE_1/Drosophila_embryo_E14-16/result/init_{0}_{1}.gz'.format(i,'ot'), pis[i], delimiter=',')
end_time = time.time()
elapsed_time = end_time - start_time
print(f"实验运行时间: {elapsed_time:.2f} 秒")
# pis = []
# for j in range(len(slices_ID)-1):
#     pi=np.loadtxt('/home/zhaoshangrui/xuxinyu/PASTE_1/Drosophila_embryo_E14-16/result/init_{0}_{1}.gz'.format(j,'ot'), delimiter=',')
#     pis.append(pi)
...
center_slice.write('/home/zhaoshangrui/xuxinyu/PASTE_1/Drosophila_embryo_E14-16/result/E14-16h_center.h5ad')
end_time = time.time()
elapsed_time = end_time - start_time
print(f"实验运行时间: {elapsed_time:.2f} 秒") 
