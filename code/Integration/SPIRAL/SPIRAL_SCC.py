...
result_dirs="/home/zhaoshangrui/xuxinyu/SPIRAL/SCC/result/"
if not os.path.exists(result_dirs+"get_input_scanpy_patient_2/"):
    os.makedirs(result_dirs+"get_input_scanpy_patient_2/") 
IDX=np.arange(0,3)
VF=[]
MAT=[]
for k in range(len(IDX)):
    ...
    sc.pp.log1p(adata)
    adata.obs['batch']=str(IDX[k])
    cells=[str(IDX[k])+'-'+i for i in adata.obs_names]
    ...
    meta1.index=cells
    meta1.to_csv(result_dirs+"get_input_scanpy_patient_2/"+str(IDX[k])+"_label-1.txt")
    coord1.to_csv(result_dirs+"get_input_scanpy_patient_2/"+str(IDX[k])+"_positions-1.txt")
    MAT.append(mat1)
...
for i in range(len(IDX)):
    ...
    mat = mat.loc[:, VF_list]  
    mat.to_csv(result_dirs + "get_input_scanpy_patient_2/" + str(IDX[i]) + "_features-1.txt")    

rad=150
KNN=6
for i in np.arange(len(IDX)):
    features=pd.read_csv(result_dirs+"get_input_scanpy_patient_2/"+str(IDX[i])+"_features-1.txt",header=0,index_col=0,sep=',')
    meta=pd.read_csv(result_dirs+"get_input_scanpy_patient_2/"+str(IDX[i])+"_label-1.txt",header=0,index_col=0,sep=',')
    coord=pd.read_csv(result_dirs+"get_input_scanpy_patient_2/"+str(IDX[i])+"_positions-1.txt",header=0,index_col=0,sep=',')
    ...
    np.savetxt(result_dirs+"get_input_scanpy_patient_2/"+str(IDX[i])+"_edge_KNN_"+str(KNN)+".csv",G_df.values[:,:2],fmt='%s')
...
for i in range(len(samples)):
    feat_file.append(result_dirs+"get_input_scanpy_patient_2/"+str(samples[i])+"_features-1.txt")
    edge_file.append(result_dirs+"get_input_scanpy_patient_2/"+str(samples[i])+"_edge_KNN_"+str(knn)+".csv")
    meta_file.append(result_dirs+"get_input_scanpy_patient_2/"+str(samples[i])+"_label-1.txt")
    coord_file.append(result_dirs+"get_input_scanpy_patient_2/"+str(samples[i])+"_positions-1.txt")
...
if not os.path.exists(result_dirs+"get_output_patient_2/"):
    os.makedirs(result_dirs+"get_output_patient_2/") 
embed_file=result_dirs+"get_output_patient_2/SPIRAL"+"_embed_"+str(SPII.params.batch_size)+".csv"
...
ann.write("/home/zhaoshangrui/xuxinyu/SPIRAL/SCC/result/ann_patient_2.h5ad")
cluster_file=result_dirs+"gtt_output_patient_2/SPIRAL_louvain.csv"
...
output_dirs=result_dirs+"get_output_patient_2/SPIRAL_alignment/"
