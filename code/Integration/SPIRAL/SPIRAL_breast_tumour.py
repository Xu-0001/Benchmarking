...
def load_slices(data_dir, slice_names):
    ...
    for slice_name in slice_names:
        ...
        slice_i.obs["batch"] = slice_i.obs["batch"].astype('category')
        # Preprocess slices
...
Batch_list = load_slices(data_dir, slice_names)
  
result_dirs="/home/zhaoshangrui/xuxinyu/SPIRAL/breast_tumour/result_new/"
...
adata.write("/home/zhaoshangrui/xuxinyu/SPIRAL/breast_tumour/result_new/ann.h5ad")

sc.pp.neighbors(adata,use_rep='spiral')
sc.tl.louvain(adata, random_state=666, key_added="louvain", resolution=1)
sc.tl.umap(adata, random_state=666)
cluster_file=result_dirs+"get_output/SPIRAL_louvain.csv"
pd.DataFrame(adata.obs['louvain']).to_csv(cluster_file)
...
adata.obsm['aligned_spatial']=adata.obsm['aligned_spatial'].astype(str)
adata.obsm['spatial'] = adata.obsm['aligned_spatial'][:,:2].astype(str)
