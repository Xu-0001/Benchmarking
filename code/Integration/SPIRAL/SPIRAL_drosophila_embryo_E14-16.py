...
result_dirs="/home/zhaoshangrui/xuxinyu/SPIRAL/Drosophila_embryoE14-16/result/"
...
for k in range(len(IDX)):
    data = adata_st_list_raw[k]
    sc.pp.highly_variable_genes(data, flavor="seurat_v3", n_top_genes=1000)
    sc.pp.normalize_total(data, target_sum=1e4)
    sc.pp.log1p(data)
    cells=[str(IDX[k])+'-'+i for i in data.obs_names]
    mat1=pd.DataFrame(data.X,columns=data.var_names,index=cells)
    coord1=pd.DataFrame(data.obsm['spatial'],columns=['x','y'],index=cells)
    meta1=data.obs[['annotation', 'slice_ID']]
    ...
    MAT.append(mat1)
    VF=np.union1d(VF,data.var_names[data.var['highly_variable']])
...
meta_file=[]
coord_file=[]
feat_file.append(result_dirs+"get_input_scanpy/"+str(samples[7])+"_features-1.txt")
edge_file.append(result_dirs+"get_input_scanpy/"+str(samples[7])+"_edge_KNN_"+str(knn)+".csv")
meta_file.append(result_dirs+"get_input_scanpy/"+str(samples[7])+"_label-1.txt")
coord_file.append(result_dirs+"get_input_scanpy/"+str(samples[7])+"_positions-1.txt")
feat_file.append(result_dirs+"get_input_scanpy/"+str(samples[8])+"_features-1.txt")
edge_file.append(result_dirs+"get_input_scanpy/"+str(samples[8])+"_edge_KNN_"+str(knn)+".csv")
meta_file.append(result_dirs+"get_input_scanpy/"+str(samples[8])+"_label-1.txt")
coord_file.append(result_dirs+"get_input_scanpy/"+str(samples[8])+"_positions-1.txt")
...
embed_file=result_dirs+"get_output/SPIRAL"+"_embed_89_"+str(SPII.params.batch_size)+".csv"
...
cluster_file=result_dirs+"get_output/SPIRAL_mclust_89.csv"
pd.DataFrame(ann.obs['mclust']).to_csv(cluster_file)

ann.write("/home/zhaoshangrui/xuxinyu/SPIRAL/Drosophila_embryoE14-16/result/ann_89.h5ad")
...
ub=['E14-16h_a_S08','E14-16h_a_S09']
...
New_Coord.to_csv(output_dirs+"new_coord_modify_89.csv")
...
ann.write("/home/zhaoshangrui/xuxinyu/SPIRAL/Drosophila_embryoE14-16/result/ann_cluster_89.h5ad")