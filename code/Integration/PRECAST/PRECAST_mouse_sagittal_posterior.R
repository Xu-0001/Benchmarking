...
slice_ids <- c("posterior1","posterior2")
seulist_split <- list()
...
saveRDS(seuList, file='/home/zhaoshangrui/xuxinyu/PRECAST/Sagittal_mouse_brain/result/Sagittal_mouse_brain_posterior_seuList.RDS')
...
write.csv(ari_precast, "/home/zhaoshangrui/xuxinyu/PRECAST/Sagittal_mouse_brain/metrice/ARI_posterior.csv", row.names = FALSE)
nmi_precast <- sapply(1:length(seuList), function(r) aricode::NMI(as.vector(PRECASTObj@resList$cluster[[r]]),
                                                                  PRECASTObj@seulist[[r]]$celltype))
write.csv(nmi_precast, "/home/zhaoshangrui/xuxinyu/PRECAST/Sagittal_mouse_brain/metrice/NMI_posterior.csv", row.names = FALSE)

PRECASTObj@seuList#NULL
seuInt <- IntegrateSpaData(PRECASTObj, species = "Mouse")
saveRDS(seuInt, file='/home/zhaoshangrui/xuxinyu/PRECAST/Sagittal_mouse_brain/result/Sagittal_mouse_brain_posterior_seuInt.RDS')

cell_embeddings <- seuInt@reductions$PRECAST@cell.embeddings
write.csv(cell_embeddings, file = "/home/zhaoshangrui/xuxinyu/PRECAST/Sagittal_mouse_brain/result/PRECAST_embed_posterior.csv", row.names = TRUE)
