...
mydata <- h5read("/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/data_mat.h5","mat")
mat <- mydata$block0_values
rownames(mat) <- as.character(mydata$axis0)
colnames(mat) <- as.character(mydata$axis1)
mat <- Matrix(mat, sparse = TRUE)
meta <- read.table("/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/data_metadata.tsv",sep="\t",header=T,row.names=1)
pos <- read.table("/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/data_position_spatial.tsv",sep="\t",header=T,row.names=1)
...
saveRDS(obj, file='/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/original_slice.RDS')
...
file_path <- '/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/original_slice.RDS'
...
seulist_split <- list()
slice_ids <- c('section_1','section_2')
...
seuList <- list()
for (r in 1:M) {
  seuList[[r]] <- CreateSeuratObject(counts = countList[[r]], meta.data = metadataList[[r]], project = "mouse brain")
}
saveRDS(seuList, file='/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/mouse_brain_seuList.RDS')


###Prepare the PRECASTObject with preprocessing step
set.seed(2024)
PRECASTObj <-  CreatePRECASTObject(seuList, project = "mouse brain")
#Add the model setting
PRECASTObj@seulist
PRECASTObj@seuList#NULL

## Add adjacency matrix list for a PRECASTObj object to prepare for PRECAST model fitting.
PRECASTObj <- AddAdjList(PRECASTObj, platform = "Visium")#more platforms to be chosen, including "Visium", "ST" and "Other_SRT"
...
saveRDS(seuInt, file='/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/mouse_brain_seuInt.RDS')

#embed file
cell_embeddings <- seuInt@reductions$PRECAST@cell.embeddings
write.csv(cell_embeddings, file = "/home/zhaoshangrui/xuxinyu/PRECAST/mouse_brain/PRECAST_embed.csv", row.names = TRUE)
