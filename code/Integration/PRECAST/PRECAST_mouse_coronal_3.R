...
slice_ids <- c("10X_Normal","10X_DAPI","10X_FFPE")
seulist_split <- list()
for (i in 1:length(slice_ids)) {
  file_path_features <- paste0("/home/zhaoshangrui/xuxinyu/SPIRAL/Coronal_mouse_brain/result/gtt_input/", slice_ids[i], "_mat.csv")
  file_path_meta <- paste0("/home/zhaoshangrui/xuxinyu/SPIRAL/Coronal_mouse_brain/result/gtt_input/", slice_ids[i], "_meta.csv")
  file_path_positions <- paste0("/home/zhaoshangrui/xuxinyu/SPIRAL/Coronal_mouse_brain/result/gtt_input/", slice_ids[i], "_coord1.csv")
...
seuList <- list()
for (r in 1:M) {
  seuList[[r]] <- CreateSeuratObject(counts = countList[[r]], meta.data = metadataList[[r]], project = "Coronal_mouse_brain")
}
saveRDS(seuList, file='/home/zhaoshangrui/xuxinyu/PRECAST/Coronal_mouse_brain/result/Coronal_mouse_brain_seuList.RDS')
...
write.csv(ari_precast, "/home/zhaoshangrui/xuxinyu/PRECAST/Coronal_mouse_brain/metrice/ARI.csv", row.names = FALSE)
nmi_precast <- sapply(1:length(seuList), function(r) aricode::NMI(as.vector(PRECASTObj@resList$cluster[[r]]),
                                                                  PRECASTObj@seulist[[r]]$celltype))
write.csv(nmi_precast, "/home/zhaoshangrui/xuxinyu/PRECAST/Coronal_mouse_brain/metrice/NMI.csv", row.names = FALSE)

PRECASTObj@seuList#NULL
seuInt <- IntegrateSpaData(PRECASTObj, species = "Mouse")
saveRDS(seuInt, file='/home/zhaoshangrui/xuxinyu/PRECAST/Coronal_mouse_brain/result/Coronal_mouse_brain_seuInt.RDS')

cell_embeddings <- seuInt@reductions$PRECAST@cell.embeddings
write.csv(cell_embeddings, file = "/home/zhaoshangrui/xuxinyu/PRECAST/Coronal_mouse_brain/result/PRECAST_embed.csv", row.names = TRUE)
